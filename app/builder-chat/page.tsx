...
const BuilderPage = () => {
  // 查询助手列表
  const { data: assistantList,loading, refresh } = useRequest(()=>chatAssistantService.listAssistants({
    page: 1,
    limit: 100
  }));
  ...
  return (
    <div className='py-8 font-sans h-screen w-screen overflow-y-auto'>
      <div className="p-2 text-center space-y-2">
        ...
        <p>
          This is a page that uses the assistant Service to get a list of helpers.
        </p>
      </div>
      <div className='flex flex-col gap-4 mx-auto px-2 max-w-[600px]'>
        ...
        <Button
          onClick={() => {
            router.push('/builder-chat/configure')
          }}
        >Create a new chat assistant</Button>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <div className='flex gap-2'>
                    ...
                    <Button
                      size={'xs'}
                      onClick={() => {
                        router.push(`/builder-chat/configure/${item.id}`)
                      }}
                    >View details</Button>
                    <Button
                      size='xs'
                      variant='destructive'
                      onClick={() => {
                        chatAssistantService.deleteAssistant(item.id).then(() => {
