...
const ConfigureContent = () => {
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      toolNames: assistant?.tools?.map((tool) => tool.type === 'function' ?
        ...
        async (e) => {
          ...
          if (await form.trigger()) {
            ...
            assistantService.updateAssistant({
              ...values,
              fileIds: values.toolNames.includes('retrieval') ? files.map((file) => file.fileId) as string[]
                : [],
              id: assistant?.id,
            }).then((res) => {
              refresh?.(); if (assistant) {
                toast.success('Update successfully');
              } else {
                router.push(`/builder/configure/${res.id}`);
              }
            })
          }
        }
